...
export class DetailsComponent implements OnInit, OnDestroy {
  ...
  constructor(public activeRoouter: ActivatedRoute, public weather: WeatherService) {
  }
  ...
  ngOnInit() {
    ...
    this.activeRoouter.paramMap.subscribe((route: any) => {

      this.city = route.params.city;
      this.sub1 = this.weather.getCurrentState(this.city).subscribe((state) => this.state = state);
      ...
      this.sub5 = this.weather.getForecast(this.city).subscribe((data: any) => {
        ...
        for (let i=0; i < data.length; i++) {
          ...
          } else if (!!this.day1Name && days[date] !== this.day1Name) {
